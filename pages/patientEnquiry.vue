        ...
        <div class="patient">
          ...
          <div class="dates">
            <div class="expected-date">
              ...
              <input type="date" id="expectedDate" v-model="data.expectedDate" class="custom-input"
                @input="validateAndCorrectDate('expectedDate')" :min="getMinDate()" />
            </div>
            <div class="expected-date">
              <label for="followUpDate">Follow-up Date</label>
              <input type="date" id="followUpDate" v-model="data.followUpDate" class="custom-input"
                @input="validateAndCorrectDate('followUpDate')" :min="getMinDate()" />
            </div>
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    validateAndCorrectDate(datatype) {
      const currentDate = new Date().toISOString().split('T')[0];
      const enteredDate = this.data[datatype];

      if (enteredDate < currentDate) {
        // Reset the input value to the current date
        this.data[datatype] = currentDate;
      } else if (enteredDate >= currentDate) {
        // Store the entered future date
        this.data[datatype] = enteredDate;
      }
    },
    updateFinance() {
    ...
    async submitForm() {
      try {
        ...
        if (!this.data.name || !this.data.paintype) {
          this.searchError = 'Please fill in the Name and Complaint fields.';
          setTimeout(() => {
            this.searchError = "";
          }, 4000);
          return;
